...
const appId = 'flux-sanity-site';
const hosts = ['sanity.ilyamoskovkin.com'];
async function startAnalytics() {
  if (typeof window === "undefined" || !hosts.includes(window.location.hostname) || navigator.doNotTrack === "1") return;
  const key = process.env.NEXT_PUBLIC_POSTHOG_KEY;
  if (!key || window.location.pathname.startsWith("/studio")) return;
  ...
  posthog.init(key, {
    ...
    before_send: (event) => {
      if (window.location.pathname.startsWith("/studio")) return null;
